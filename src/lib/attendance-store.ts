...
let recentCache: { workerId: string; date: string; savedAt: string }[] = [];
let loaded = false;
...
export async function refreshAttendance(): Promise<void> {
  ...
  const byDate = new Map<string, { ids: Set<string>; savedAt: string }>();
  for (const row of (data ?? []) as {
    ...
    if (row.created_at > entry.savedAt) entry.savedAt = row.created_at;
    byDate.set(row.attendance_date, entry);
  }

  cache = [...byDate.entries()]
    .map(([date, entry]) => ({ date, workerIds: [...entry.ids], savedAt: entry.savedAt }))
  ...
  recentCache = ((data ?? []) as {
    ...
    .map((row) => ({ workerId: row.worker_id, date: row.attendance_date, savedAt: row.created_at }))
...
export function useAttendance() {
   ...
   * The database enforces one row per worker per date, so a worker can never be
   * counted twice on the same day.
   */
  const saveAttendance = useCallback(async (date: string, workerIds: string[]) => {
    ...
    const existing = new Set(
      ((existingRows ?? []) as { worker_id: string }[]).map((r) => r.worker_id),
    );
    ...
    if (toAdd.length > 0) {
      const { error } = await supabase
        .from("attendance")
        .upsert(
          toAdd.map((worker_id) => ({ worker_id, attendance_date: date })),
          { onConflict: "worker_id,attendance_date", ignoreDuplicates: true },
        );
      if (error) throw new Error(error.message);
    }
  ...
  const getRecordForDate = useCallback(
    (date: string): AttendanceRecord | undefined => records.find((r) => r.date === date),
    [records],
  );

  return { records, recent, hydrated, saveAttendance, getRecordForDate };
}

/** Present-worker count for a date — lets the Dashboard read "Worked Today". */
export function countPresentOn(records: AttendanceRecord[], date: string): number {
  return records.find((r) => r.date === date)?.workerIds.length ?? 0;
}
...
export function countInMonth(
  ...
  return records
    ...
    .reduce((sum, r) => sum + r.workerIds.length, 0);
}
